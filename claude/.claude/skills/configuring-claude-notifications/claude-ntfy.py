...
def log_error(msg: str) -> None:
    ...
    with open(ERROR_LOG, "a") as f:
        f.write("[{}] {}\n".format(__import__('time').time(), msg))
...
class NtfyNotifier:
    ...
    def _load_config(self) -> None:
        ...
        if not self.CONFIG_PATH.exists():
            raise FileNotFoundError("設定ファイルが見つかりません: {}".format(self.CONFIG_PATH))

        with open(self.CONFIG_PATH, "r") as f:
            data = json.load(f)

        self._ntfy_url = data["ntfy_url"]
        # 設定ファイルからデバッグモードを読み込み（環境変数より優先）
        if "debug_mode" in data:
            self._debug_mode = data["debug_mode"]

        log_debug("デバッグモード: {}".format(self._debug_mode))
        log_debug("設定ファイルを読み込み: {}".format(self.CONFIG_PATH))
